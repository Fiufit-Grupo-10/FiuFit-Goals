...
from app.api.goals.models import (
    Goal,
    UserGoals
)
...
async def create_user_goals(user_id: str,goals: list[Goal], request: Request):
    new_goals = await crud.create_user_goals(user_id=user_id,goals=goals,request=request)    
    return JSONResponse(status_code=status.HTTP_201_CREATED, content=new_goals)
